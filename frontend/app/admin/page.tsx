  ...
  return (
    <main className="min-h-screen bg-gray-100">
      <header className="bg-black text-white px-8 py-4 flex justify-between items-center">
        <div>
          <h1 className="text-xl font-bold">Cape Parts Finder - Admin</h1>
          <p className="text-xs text-gray-400">Admin Dashboard</p>
        </div>
        <div className="flex gap-4">
          <button onClick={exportToCSV} className="bg-green-500 text-white px-4 py-2 rounded-xl text-sm">Export CSV</button>
          <button onClick={logout} className="bg-red-500 text-white px-4 py-2 rounded-xl text-sm">Logout</button>
        </div>
      </header>
      ...
      <div className="max-w-7xl mx-auto p-8">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          ...
          <div className="bg-blue-50 p-4 rounded-2xl shadow text-center">
            <p className="text-3xl font-bold text-blue-600">{counts.New}</p>
            <p className="text-gray-500 text-sm">New</p>
          </div>
          <div className="bg-yellow-50 p-4 rounded-2xl shadow text-center">
            <p className="text-3xl font-bold text-yellow-600">{counts.Searching + counts.Quoted}</p>
        ...
        <div className="flex flex-col md:flex-row gap-4 mb-6">
          <input type="text" placeholder="Search..." value={search} onChange={(e) => setSearch(e.target.value)} className="flex-1 p-4 rounded-xl border" />
          <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)} className="p-4 rounded-xl border">
            ...
            <div key={request.id} className="bg-white p-6 rounded-2xl shadow border">
              <div className="flex justify-between items-start mb-4">
                <div>
                  ...
                  <p className="text-gray-400 text-xs">{new Date(request.created_at).toLocaleString("en-ZA")}</p>
                </div>
                <div className="flex gap-3">
                  <select value={request.status || "New"} onChange={(e) => updateStatus(request.id, e.target.value)} className="border p-2 rounded-xl text-sm">
                    <option>New</option>
                  ...
                  <button onClick={() => deleteRequest(request.id)} className="text-red-400 text-sm px-3 py-2 border border-red-200 rounded-xl">Delete</button>
                </div>
              </div>
              ...
              <div className="mt-4 flex gap-3">
                <button onClick={() => window.open("https://wa.me/" + request.phone_number.replace(/\D/g, ""))} className="bg-green-500 text-white px-4 py-2 rounded-xl text-sm">WhatsApp</button>
                <button onClick={() => window.open("mailto:" + request.email)} className="bg-blue-500 text-white px-4 py-2 rounded-xl text-sm">Email</button>
              </div>

              {request.photo_url && (
                <div className="mt-4">
                  <p className="font-bold text-sm mb-2">Photo:</p>
                  <img src={request.photo_url} alt="Uploaded" className="w-64 rounded-xl border" />
                </div>
              )}
            </div>
          ))}
