...
export default function RemindersPage() {
  ...
  function isCriticalOverdue(remind_at: string) {
    return (now.getTime() - new Date(remind_at).getTime()) > 24 * 60 * 60 * 1000;
  }
        ...
        <div className="max-w-4xl mx-auto px-4 py-3 flex items-center justify-between gap-3">
          ...
          <div className="flex gap-1 overflow-x-auto scrollbar-hide">
            {[
              ...
              { label: "Expenses", href: "/expenses" }, { label: "Reminders", href: "/reminders" }, { label: "Settings", href: "/settings" }, { label: "Reminders", href: "/reminders", active: true },
            ].map((n) => (
        ...
        {filtered.length === 0 ? (
          ...
          <div className="space-y-2">
            {filtered.map(r => {
              ...
              const critical = !r.completed && isCriticalOverdue(r.remind_at);
              const today = !r.completed && isToday(r.remind_at);
              return (
                <div key={r.id} style={{
                  ...cardStyle, ...(critical ? {border:'1px solid rgba(239,68,68,0.6)',background:'rgba(239,68,68,0.05)'} : overdue ? {border:'1px solid rgba(249,115,22,0.4)'} : {}),
                    ...
                    <div className="flex items-center gap-2 flex-shrink-0">
                      {!r.completed && (
                        <>
                          ...
                          {critical && <a href={`https://wa.me/27696863952?text=${encodeURIComponent("URGENT: Reminder overdue 24h+ for " + r.customer_name + " - " + (r.note||""))}`} target="_blank" style={{background:"rgba(239,68,68,0.15)",border:"1px solid rgba(239,68,68,0.3)",color:"#ef4444",padding:"6px 12px",borderRadius:8,fontSize:13,fontWeight:"bold",textDecoration:"none",marginRight:8}}>Alert Me</a>}{critical && <a href={`https://wa.me/27696863952?text=${encodeURIComponent("URGENT: Reminder overdue 24h+ for " + r.customer_name + " - " + (r.note||""))}`} target="_blank" style={{background:"rgba(239,68,68,0.15)",border:"1px solid rgba(239,68,68,0.3)",color:"#ef4444",padding:"6px 12px",borderRadius:8,fontSize:13,fontWeight:"bold",textDecoration:"none",marginRight:8}}>Alert Me</a>}<button onClick={() => markDone(r.id)} disabled={completing === r.id}
