...
export default function Home() {
  ...
  if (success) {
    return (
      <main style={{ background: "linear-gradient(135deg, #0f0f0f 0%, #1a1a2e 50%, #16213e 100%)" }} className="min-h-screen flex items-center justify-center p-6">
        <div className="text-center">
          ...
          <p className="text-gray-400 text-sm leading-relaxed mb-8 max-w-sm mx-auto">
            Request received for <span className="text-orange-400 font-semibold">{formData.part_needed}</span>. We'll WhatsApp you on <span className="text-white font-semibold">{formData.phone_number}</span>.
          </p>
      ...
      <main className="min-h-screen bg-[#F8F8F7]">
        <div className="bg-white border-b border-gray-100 sticky top-0 z-10">
          <div className="max-w-lg mx-auto px-5 h-14 flex items-center justify-between">
            ...
            <div className="flex items-center gap-1.5">
              ...
              <span className="font-bold text-gray-900 text-[14px]">Cape Parts Finder</span>
            </div>
            <div className="w-12" />
          </div>
        </div>
        ...
        <div className="max-w-lg mx-auto px-4 py-6 pb-16">
          <div className="mb-5">
            ...
            <p className="text-sm text-gray-400 mt-0.5">Fill in below — we'll WhatsApp you a quote.</p>
          </div>
          <form onSubmit={handleSubmit} className="space-y-3">
            <div className="bg-white rounded-2xl border border-gray-100 p-5 space-y-3">
              <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Contact Info</p>
              <input name="customer_name" placeholder="Full Name *" value={formData.customer_name} onChange={handleChange} required className="w-full border border-gray-100 bg-gray-50 rounded-xl px-4 py-3 text-[14px] outline-none focus:border-orange-400 focus:bg-white transition" />
              <input name="phone_number" placeholder="WhatsApp Number *" value={formData.phone_number} onChange={handleChange} required type="tel" className="w-full border border-gray-100 bg-gray-50 rounded-xl px-4 py-3 text-[14px] outline-none focus:border-orange-400 focus:bg-white transition" />
              <div className="grid grid-cols-2 gap-2">
                <input name="email" placeholder="Email (optional)" value={formData.email} onChange={handleChange} type="email" className="border border-gray-100 bg-gray-50 rounded-xl px-4 py-3 text-[14px] outline-none focus:border-orange-400 focus:bg-white transition" />
                <input name="area" placeholder="Your Area" value={formData.area} onChange={handleChange} className="border border-gray-100 bg-gray-50 rounded-xl px-4 py-3 text-[14px] outline-none focus:border-orange-400 focus:bg-white transition" />
              </div>
            </div>
            <div className="bg-white rounded-2xl border border-gray-100 p-5 space-y-3">
              <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Vehicle</p>
              <div className="grid grid-cols-3 gap-2">
                <input name="vehicle_make" placeholder="Make" value={formData.vehicle_make} onChange={handleChange} className="border border-gray-100 bg-gray-50 rounded-xl px-3 py-3 text-[14px] outline-none focus:border-orange-400 focus:bg-white transition" />
                <input name="vehicle_model" placeholder="Model" value={formData.vehicle_model} onChange={handleChange} className="border border-gray-100 bg-gray-50 rounded-xl px-3 py-3 text-[14px] outline-none focus:border-orange-400 focus:bg-white transition" />
                <input name="vehicle_year" placeholder="Year" value={formData.vehicle_year} onChange={handleChange} type="number" className="border border-gray-100 bg-gray-50 rounded-xl px-3 py-3 text-[14px] outline-none focus:border-orange-400 focus:bg-white transition" />
              </div>
              <div className="grid grid-cols-2 gap-2">
                <input name="vin_number" placeholder="VIN Number" value={formData.vin_number} onChange={handleChange} className="border border-gray-100 bg-gray-50 rounded-xl px-4 py-3 text-[14px] outline-none focus:border-orange-400 focus:bg-white transition" />
                <input name="engine_size" placeholder="Engine Size" value={formData.engine_size} onChange={handleChange} className="border border-gray-100 bg-gray-50 rounded-xl px-4 py-3 text-[14px] outline-none focus:border-orange-400 focus:bg-white transition" />
              </div>
            </div>
            <div className="bg-white rounded-2xl border border-gray-100 p-5 space-y-3">
              <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Part Details</p>
              <input name="part_needed" placeholder="What part do you need? *" value={formData.part_needed} onChange={handleChange} required className="w-full border border-gray-100 bg-gray-50 rounded-xl px-4 py-3 text-[14px] outline-none focus:border-orange-400 focus:bg-white transition" />
              <div>
                <p className="text-[11px] text-gray-400 mb-2">Preference</p>
                <div className="flex gap-2">
                  {[{ val: "aftermarket", label: "Aftermarket" }, { val: "original", label: "Original OEM" }, { val: "either", label: "Either" }].map((p) => (
                    <button key={p.val} type="button" onClick={() => setFormData({ ...formData, part_preference: p.val })}
                      className={`flex-1 py-2 rounded-xl text-[12px] font-medium border transition cursor-pointer ${formData.part_preference === p.val ? "bg-gray-900 border-gray-900 text-white" : "bg-gray-50 border-gray-100 text-gray-500 hover:border-gray-300"}`}>
                      {p.label}
                    </button>
                  ))}
                </div>
              </div>
              <textarea name="extra_details" placeholder="Extra details — colour, side, condition (optional)" value={formData.extra_details} onChange={handleChange} rows={2} className="w-full border border-gray-100 bg-gray-50 rounded-xl px-4 py-3 text-[14px] outline-none focus:border-orange-400 focus:bg-white transition resize-none" />
              <div>
                <p className="text-[11px] text-gray-400 mb-2">Photo (optional)</p>
                <div onClick={() => document.getElementById("photo-input")?.click()} className="border border-dashed border-gray-200 rounded-xl p-4 cursor-pointer hover:border-orange-300 transition bg-gray-50 hover:bg-white">
                  {photo ? (
                    <div className="flex items-center gap-3">
                      <img src={URL.createObjectURL(photo)} alt="Preview" className="w-11 h-11 object-cover rounded-lg" />
                      <div><p className="text-[13px] font-medium text-gray-700 truncate max-w-[180px]">{photo.name}</p><p className="text-[11px] text-gray-400">Tap to change</p></div>
                    </div>
                  ) : (
            ...
            <button type="submit" disabled={loading} className="w-full bg-orange-500 hover:bg-orange-600 disabled:bg-orange-300 text-white py-4 rounded-2xl font-bold text-[15px] transition cursor-pointer">
  ...
  return (
    <main className="min-h-screen overflow-hidden" style={{ background: "#0a0a0a" }}>
      ...
      <div className="fixed inset-0 pointer-events-none" style={{ zIndex: 0 }}>
        ...
        <div style={{ position: "absolute", top: "40%", left: "40%", width: "400px", height: "400px", borderRadius: "50%", background: "radial-gradient(circle, rgba(139,92,246,0.06) 0%, transparent 70%)" }} />
      </div>
      ...
      <div style={{ position: "relative", zIndex: 1 }}>
        ...
        <nav className="px-6 py-5 flex items-center justify-between max-w-6xl mx-auto">
          ...
          <a href="/login" className="text-[13px] text-gray-500 hover:text-gray-300 transition no-underline border border-gray-800 hover:border-gray-600 px-3.5 py-1.5 rounded-lg">Admin</a>
        </nav>

        {/* Hero */}
        <div className="max-w-6xl mx-auto px-6 pt-16 pb-8 lg:pt-24">
          <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full text-[11px] font-semibold mb-8 border" style={{ background: "rgba(249,115,22,0.1)", borderColor: "rgba(249,115,22,0.2)", color: "#fb923c" }}>
            <span className="w-1.5 h-1.5 rounded-full bg-orange-400 animate-pulse" />
            Serving Cape Town · Fast Turnaround
          </div>

          <h1 className="text-[52px] lg:text-[72px] font-black leading-[1.0] tracking-tight mb-6" style={{ color: "#ffffff" }}>
          ...
          {/* Stats */}
          <div className="flex gap-8 pb-16 border-b" style={{ borderColor: "rgba(255,255,255,0.06)" }}>
        ...
        <div className="max-w-6xl mx-auto px-6 py-16">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {[
              { num: "01", title: "Submit your request", desc: "Tell us the part you need and your vehicle details. Takes under 2 minutes.", highlight: true },
              { num: "02", title: "We source it", desc: "Our team contacts our verified Cape Town suppliers to find the best price and availability." },
              { num: "03", title: "Get quoted on WhatsApp", desc: "We send you a quote on WhatsApp. Confirm and we arrange delivery or collection." },
            ].map((s) => (
              <div key={s.num} className="rounded-2xl p-6 border" style={{ background: s.highlight ? "rgba(249,115,22,0.08)" : "rgba(255,255,255,0.03)", borderColor: s.highlight ? "rgba(249,115,22,0.2)" : "rgba(255,255,255,0.06)" }}>
                <div className="text-orange-500 font-bold text-[12px] mb-4 font-mono">{s.num}</div>
        ...
        <div className="max-w-6xl mx-auto px-6 pb-16">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {[
              ...
              <div key={item.title} className="rounded-2xl p-5 border" style={{ background: "rgba(255,255,255,0.03)", borderColor: "rgba(255,255,255,0.06)" }}>
        ...
        {/* CTA */}
        <div className="max-w-6xl mx-auto px-6 pb-16">
          <div className="rounded-3xl p-10 text-center" style={{ background: "linear-gradient(135deg, rgba(249,115,22,0.15) 0%, rgba(249,115,22,0.05) 100%)", border: "1px solid rgba(249,115,22,0.2)" }}>
        ...
        {/* Footer */}
        <div className="border-t py-6 px-6 text-center max-w-6xl mx-auto" style={{ borderColor: "rgba(255,255,255,0.06)" }}>
