  ...
  return (
    <main className="min-h-screen bg-[#FAFAF9]">
      ...
      <div className="max-w-7xl mx-auto px-6 py-7">
        ...
        {requestId && (
          <div className="bg-orange-50 border border-orange-200 rounded-2xl p-4 mb-6 flex items-start gap-4">
            ...
            <button onClick={() => router.push("/admin")}
              className="text-orange-400 hover:text-orange-700 text-lg font-bold cursor-pointer bg-transparent border-none leading-none">
              ×
            </button>
          </div>
        )}
        ...
        <div className="grid xl:grid-cols-2 gap-4">
          {filteredSuppliers.map((supplier) => (
            <div key={supplier.id} className={`bg-white rounded-2xl border shadow-sm overflow-hidden transition ${requestId ? "border-orange-100 hover:border-orange-300" : "border-gray-100"}`}>
              ...
              <div className="p-5">
                ...
                {/* Send Quote button - highlighted when coming from a request */}
                <button onClick={() => openQuoteModal(supplier)}
                  className={`w-full flex items-center justify-center gap-2 py-2.5 rounded-lg text-[13px] font-semibold transition cursor-pointer mb-2 border ${
                    requestId
          ...
          <div className="bg-white rounded-2xl w-full max-w-md shadow-xl">
            ...
            <div className="p-5 space-y-4">
              {quoteSuccess ? (
                ...
                <>
                  {/* Show pre-filled request details if from admin */}
                  {requestId && (
                    <div className="bg-orange-50 border border-orange-100 rounded-xl p-3">
                      <p className="text-[11px] font-semibold text-orange-700 uppercase tracking-wide mb-1">Customer Request</p>
                      <p className="text-sm text-gray-700 font-medium">{requestPart}</p>
                      <p className="text-[12px] text-gray-500">{requestYear} {requestMake} {requestModel}</p>
                    </div>
                  )}
                  ...
                  {/* Note - pre-filled with request details */}
                  <div>
                    <label className="text-[12px] font-semibold text-gray-500 uppercase tracking-wide block mb-2">
                      Note to Supplier
                    </label>
